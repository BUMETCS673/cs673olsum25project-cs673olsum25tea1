...
class ActivityTest {
	...
	void given_user_who_activity_post_requested_without_admin_permission_then_403_returned() throws Exception {
		...
		Map<String, String> createActivityReq = Map.of("name", "Test Activity", "location", "Test Location",
				"description", "Description", "startDateTime", "2011-12-03 10:15", "endDateTime", "2011-12-03 11:15");
		...
		/*
		 * requestDto = new RegistrationRequestDto("user2@bu.edu", "user2",
		 * "testpassword"); String u2token = getToken(mockMvc, requestDto);
		 * 
		 * // When user2 is not admin of user1's activity, tries to update the activity,
		 * 403 should be returned. Map<String, String> updateActivityReq = Map.of(
		 * "activityId", activityId, "name", "Updated Activity Name" ); sendPut(mockMvc,
		 * RestEndpoint.ACTIVITY, updateActivityReq, u2token)
		 * .andExpect(status().isForbidden()) .andDo(print())
		 * .andExpect(jsonPath("$.errors").exists())
		 * .andExpect(jsonPath("$.errors.errorCode").value(ErrorCode.
		 * RESOURCE_ACCESS_DENIED.getCode()))
