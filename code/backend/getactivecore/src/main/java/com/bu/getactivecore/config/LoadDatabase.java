	...
	CommandLineRunner demoPreloadData(ActivityRepository activityRepo, UserRepository userRepo) {
		return args -> {
			Activity act1 = Activity.builder().name("Rock Climbing").startDateTime(LocalDateTime.now())
					.location("Location").endDateTime(LocalDateTime.now()).build();
			Activity act2 = Activity.builder().name("Yoga").location("Location").startDateTime(LocalDateTime.now())
					.endDateTime(LocalDateTime.now()).build();
			Activity act3 = Activity.builder().name("Running").location("Location").startDateTime(LocalDateTime.now())
					.endDateTime(LocalDateTime.now()).build();

			Users user1 = Users.builder().email("arsh@").username("arsh").password("arsh")
					.accountState(AccountState.UNVERIFIED).build();
			Users user2 = Users.builder().email("arsh2@").username("arsh2").password("arsh2")
