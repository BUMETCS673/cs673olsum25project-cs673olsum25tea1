...
public class ApiInternalException extends ApiException {
	...
	/**
	 * Generic internal server error exception thrown when an unexpected error
	 * occurs.
	 *
	...
	public ApiInternalException(String message) {
		super(message);
		error = ApiErrorPayload.builder().status(HttpStatus.INTERNAL_SERVER_ERROR)
				.errorCode(ErrorCode.INTERNAL_SERVER_ERROR).message(message).build();
	}
